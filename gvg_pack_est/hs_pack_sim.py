...
def open_pack(p_upgrade = .2):
	...
	card_level = -1
	upgrade = 1
	p_upgrade = round(p_upgrade, 3)
	upgrade_choice_list = [1] * int(1000 * p_upgrade) + [0] * int(1000 - 1000 * p_upgrade)
	
	for card in range(5):
	
		while upgrade == 1 and card_level < 3:
			upgrade = random.choice(upgrade_choice_list)
			card_level += 1
...
def pack_to_cards(opened_pack, n_common = 40, n_rare = 37, n_epic = 26, n_legendary = 20):
	n_total = n_common + n_rare + n_epic + n_legendary
	card_list = []
	for card_level in opened_pack:
		if card_level == 0:
			...
			card = random.choice(range(1 + n_common + n_rare + n_epic, 1 + n_total))
		else:
...
class Collection(object):
	...
	dust_cost = {'comm': 40, 'rare': 100, 'epic': 400, 'leg': 1600}
	...
	def __init__(self, all_cards, pct_bad_cards = [0,0,0,0], dust = 0, p_upgrade = .2):
	...
	def add_pack(self, pack_of_cards):
		for card in pack_of_cards:
			...
			if card in range(self.n_total + 1, self.n_total + 5): #is is golden? if so, dust it
				...
				if not card in self.all_cards: # don't have the card yet?
					...
					if num_of_card == 1: # 
						if card > self.n_total - self.n_leg: # if it's a legendary
							...
							self.missing_cards[card] = 0
					else:
						self.dust_card(card)

	def open_packs(self, num = 40):
		for pack in range(num):
			self.add_pack(pack_to_cards(open_pack(self.p_upgrade)))
		#self.print_collection()

	def return_missing_cards(self):
		missing_cards = {}
		for card in range(1, self.n_total - self.n_leg + 1):
		...
		for card in range(self.n_total - self.n_leg + 1, self.n_total + 1):
			if card in self.bad_cards:
				pass
			elif card in self.all_cards:
...
def replicate_buy_complete_set(n, pct_bad_cards = [0,0,0,0], p_upgrade = .2):
	n_packs_list = []
	for i in range(n):
		# print(i)
		n_packs_list.append(Collection({}, pct_bad_cards = pct_bad_cards, p_upgrade = p_upgrade).buy_complete_set())
	return(n_packs_list)

def mean(list):
	return(sum(list)/len(list))

def rarity_dist(n = 1000, p_upgrade = .2):
	n_list = [40, 37, 26, 20]
	n_opened = [0]*8
	for i in range(n):
		pack = open_pack(p_upgrade)
		for card in pack:
			n_opened[card] += 1
	pct = [round(x/sum(n_opened),5) for x in n_opened]
	return(pct)

# exec(open("C:\\Python34\\Scripts\\hs\\gvg_pack_est\\hs_pack_sim.py").read())
